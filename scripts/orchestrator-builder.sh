...
function gen_manifests {
    ...
    if [[ -f "$res_dir_path/secret.properties" ]]; then
        ...
        koc create secret generic "${workflow_id}-creds" "${create_secret_args[@]}" > "${args["manifests-directory"]}/00-secret_${workflow_id}.yaml"
...
function build_image {
    ...
    local pocker_args=(
        -f="$script_dir_path/docker/orchestrator.Dockerfile"
        --tag="${args["image"]}"
